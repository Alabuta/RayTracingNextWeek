...
template<class T>
gfx::buffer<T> create_buffer(std::uint32_t binding_index, std::uint32_t size)
{
    std::uint32_t handle;
    ...
    auto size_in_bytes = static_cast<std::ptrdiff_t>(sizeof(T) * size);
    ...
    return gfx::buffer<T>{handle, binding_index, size};
}

template<class T>
void update_buffer(gfx::buffer<T> const &buffer, T const *data)
{
    auto size_in_bytes = static_cast<std::ptrdiff_t>(sizeof(T) * buffer.size);
