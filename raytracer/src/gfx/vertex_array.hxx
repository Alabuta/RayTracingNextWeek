...
namespace gfx
{
    ...
    template<std::int32_t N, class T>
    gfx::vertex_array<N, T>
    create_vertex_array(std::uint32_t attribute_index, gfx::shader_storage_buffer<glm::vec<N, T>> const &buffer)
    {
        ...
        if constexpr (std::is_same_v<T, float>)
            ...
            throw std::runtime_error("unsupported attribute type"s);
