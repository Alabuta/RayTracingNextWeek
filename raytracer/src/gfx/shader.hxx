...
namespace gfx
{
    using shader_stage = std::variant<shader::vertex, shader::tess_control, shader::tess_eval, shader::geometry, shader::fragment, shader::compute>;
...
#pragma GCC diagnostic pop
        ...
        glShaderBinary(1, &shader_stage.handle, GL_SHADER_BINARY_FORMAT_SPIR_V, std::data(byte_code), static_cast<std::int32_t>(std::size(byte_code)));
...
namespace gfx
{
    ...
    static gfx::program create_program(std::vector<gfx::shader_stage> const &shader_stages)
    {
        using namespace std::string_literals;
        using namespace std::string_view_literals;
